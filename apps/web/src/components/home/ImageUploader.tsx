    ...
    const onDrop = useCallback(
        async (acceptedFiles: File[]) => {
            ...
            if (file) {
                setLoading(true);
                try {
                    // 1. Get the raw result from the backend
                    const backendResult = await identifyCard(file);
                    // 2. Transform the data to match the frontend's PokemonCard type
                    const frontendCard = {
                        id: backendResult.id,
                        name: backendResult.name,
                        set: {
                            name: backendResult.set, // Assuming backendResult.set is a string like "Base Set"
                            number: backendResult.id, // Or another field
                        },
                        rarity: backendResult.rarity,
                        imageUrl: backendResult.official_card_image_url,
                        estimatedPrice: 0, // Not provided by backend yet
                    };
                    // 3. Create query with the correctly formatted object
                    const query = new URLSearchParams({
                        card: JSON.stringify(frontendCard), // Use the transformed object
        ...
        <Card className="p-6">
            <div className="text-center mb-4">
                ...
                <p className="text-sm text-text-secondary">
                    Upload a photo of your Pokémon card to identify it instantly.
                </p>
            </div>
            <div
                {...getRootProps()}
